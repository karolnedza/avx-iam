...
resource "aviatrix_account" "avx_iam_account_1" {
  account_name       = "avx-aws-some-account_1"
  cloud_type         = 1
  aws_account_number = module.aviatrix-iam-roles.aws-account
  aws_iam            = true
  aws_role_app       = time_sleep.iam_resource_propagation.triggers["app_arn"] # AWS tends to be slow with creating and propageting IAM roles 
